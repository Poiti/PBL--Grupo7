...
calData calib3 = { 0 };     // Calibration data for IMU2
AccelData accelData3;       // Accelerometer data for IMU2
GyroData gyroData3;         // Gyroscope data for IMU2
MagData magData3;           // Magnetometer data for IMU2

const char* ssid = "poit";
const char* password = "espaaaaa";
...
IMU3.update();
IMU3.getAccel(&accelData3);
IMU3.getGyro(&gyroData3);
if (IMU3.hasMagnetometer()) IMU3.getMag(&magData2);
...
JsonObject imu3 = jsonDoc.createNestedObject("IMU3");
JsonObject imu3Accel = imu3.createNestedObject("Accel");
imu3Accel["X"] = accelData3.accelX;
imu3Accel["Y"] = accelData3.accelY;
imu3Accel["Z"] = accelData3.accelZ;

JsonObject imu3Gyro = imu3.createNestedObject("Gyro");
imu3Gyro["X"] = gyroData3.gyroX;
imu3Gyro["Y"] = gyroData3.gyroY;
imu3Gyro["Z"] = gyroData3.gyroZ;

if (IMU3.hasMagnetometer()) {
  JsonObject imu3Mag = imu3.createNestedObject("Mag");
  imu3Mag["X"] = magData3.magX;
  imu3Mag["Y"] = magData3.magY;
  imu3Mag["Z"] = magData3.magZ;
...
void setup() {
  ...
  if (IMU3.init(calib3, IMU_ADDRESS2) != 0) {
    Serial.println("Error initializing IMU3");
    while (true);
  }
...
void loop() {
  ...
  if (client) {
    ...
    while (client.connected()) {
      ...
      static uint32_t prev_ms = millis();
      if (millis() > prev_ms + 500) { // Atualiza a cada 25ms
        ...
        String sensorData = "Sensor 1 acel - x=" + String(accelData1.accelX) +
                            ...
                            " z=" + String(accelData2.accelZ)+ " Sensor 3 acel - x="+ String(accelData3.accelX)  +" y=" + String(accelData3.accelY) + 
                            " z=" + String(accelData3.accelZ);
